...
const expected = {
    operations: [
        ...
        {
            Name: "NewPets",
            Content: "subscription NewPets @internalOperation {\n  newPets {\n    name\n  }\n}",
...
test("parseGraphQLOperations", () => {
    const actual = (0, operations_1.parseOperations)(testSchema, operations);
...
test("parse operations with fromClaim directive", () => {
    const actual = (0, operations_1.parseOperations)(fromClaimSchema, fromClaimOperations);
    chai_1.assert.equal(pretty(actual), pretty(fromClaimParsed));
});
test("parse operations with jsonSchema directive", () => {
    const actual = (0, operations_1.parseOperations)(jsonSchemaDirectiveSchema, jsonSchemaDirectiveOperation);
    chai_1.assert.equal(pretty(actual), pretty(jsonSchemaDirectiveParsed));
});
...
const jsonSchemaDirectiveOperationParsed = (0, graphql_1.print)((0, graphql_1.parse)(jsonSchemaDirectiveOperation));
const jsonSchemaDirectiveParsed = {
    operations: [
        {
            Name: "JsonSchemaQuery",
            Content: jsonSchemaDirectiveOperationParsed.substring(0, jsonSchemaDirectiveOperationParsed.length - 1),
            OperationType: wundernode_config_1.OperationType.QUERY,
            VariablesSchema: {
                type: "object",
                properties: {
...
const subTest = (rawSchema, testCase) => {
    const schema = (0, graphql_1.buildSchema)(rawSchema);
    testCase.forEach((testCase, i) => {
        const queryDocument = (0, graphql_1.parse)(testCase.query);
        const operation = queryDocument.definitions.find(node => node.kind === "OperationDefinition");
        const actual = (0, operations_1.operationResponseToJSONSchema)(schema, queryDocument, operation);
        chai_1.assert.equal(pretty(actual), pretty(testCase.expected), "testCase: " + i);
        if (testCase.expectedVariables === undefined) {
            return;
        }
        const actualVariables = (0, operations_1.operationVariablesToJSONSchema)(schema, operation);
